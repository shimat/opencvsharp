...
- **`bool` + `out` array signatures dropped in favor of a direct return value** (empty array/buffer signals failure, matching the existing `ImRead`/`ImDecode` convention): `Cv2.ImReadMulti`, `Cv2.ImDecodeMulti`, `Cv2.ImEncode`, `Cv2.ImEncodeAnimation`, `Cv2.ImEncodeWithMetadata`, `Cv2.ImEncodeMulti`, `Cv2.FindChessboardCorners`/`FindChessboardCornersSB`/`FindCirclesGrid` (the `out Point2f[]`-returning overloads only — the `OutputArray`-based overloads are unchanged), `QRCodeDetector.Detect`/`DetectMulti`/`DecodeMulti`, `ObjectnessBING.ComputeSaliency`, `Facemark.Fit`, `FontFace.GetInstance`.
