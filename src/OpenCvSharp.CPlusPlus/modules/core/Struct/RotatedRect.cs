...
namespace OpenCvSharp.CPlusPlus
{
    ...
    public struct RotatedRect
    {
        ...
        public Rect BoundingRect()
        {
            Point2f[] pt = Points();
            Rect r = new Rect{
                X = Cv.Floor(Math.Min(Math.Min(Math.Min(pt[0].X, pt[1].X), pt[2].X), pt[3].X)),
                Y = Cv.Floor(Math.Min(Math.Min(Math.Min(pt[0].Y, pt[1].Y), pt[2].Y), pt[3].Y)),
                Width = Cv.Ceil(Math.Max(Math.Max(Math.Max(pt[0].X, pt[1].X), pt[2].X), pt[3].X)),
                Height = Cv.Ceil(Math.Max(Math.Max(Math.Max(pt[0].Y, pt[1].Y), pt[2].Y), pt[3].Y))
            };
            r.Width -= r.X - 1;
            r.Height -= r.Y - 1;
            return r;
