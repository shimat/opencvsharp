...
{
	cv::gpu::boxFilter(*src, *dst, ddepth, ksize, anchor, entity(stream));
}
...
{
	cv::gpu::erode(*src, *dst, *kernel, *buf, anchor, iterations, entity(stream));
}
...
CVAPI(void) gpu_dilate2(
	...
	cv::Point anchor, int iterations, Stream *stream)
{
	cv::gpu::dilate(*src, *dst, *kernel, *buf, anchor, iterations, entity(stream));
}
...
CVAPI(void) gpu_morphologyEx2(
	...
	cv::Point anchor, int iterations, Stream *stream)
{
	cv::gpu::morphologyEx(*src, *dst, op, *kernel, *buf1, *buf2, anchor, iterations, entity(stream));
}

CVAPI(void) gpu_filter2D(
	GpuMat *src, GpuMat *dst, int ddepth, cv::Mat *kernel, cv::Point anchor, int borderType, Stream *stream)
{
	cv::gpu::filter2D(*src, *dst, ddepth, *kernel, anchor, borderType, entity(stream));
}
...
{
	cv::gpu::sepFilter2D(*src, *dst, ddepth, *kernelX, *kernelY, *buf, anchor, rowBorderType, columnBorderType, entity(stream));
}
...
{
	cv::gpu::Sobel(*src, *dst, ddepth, dx, dy, *buf, ksize, scale, rowBorderType, columnBorderType, entity(stream));
}
...
{
	cv::gpu::Scharr(*src, *dst, ddepth, dx, dy, *buf, scale, rowBorderType, columnBorderType, entity(stream));
}
...
CVAPI(void) gpu_GaussianBlur2(
	...
	int rowBorderType, int columnBorderType, Stream *stream)
{
	cv::gpu::GaussianBlur(*src, *dst, ksize, *buf, sigma1, sigma2, rowBorderType, columnBorderType, entity(stream));
}

CVAPI(void) gpu_Laplacian(
	GpuMat *src, GpuMat *dst, int ddepth, int ksize, double scale, int borderType, Stream *stream)
{
	cv::gpu::Laplacian(*src, *dst, ddepth, ksize, scale, borderType, entity(stream));
}
