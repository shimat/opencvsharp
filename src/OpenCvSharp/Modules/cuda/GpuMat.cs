...
namespace OpenCvSharp.Cuda
{
    ...
    public class GpuMat : CvObject
    {
        ...
        public GpuMat(int rows, int cols, MatType type)
        {
            ...
            var p = NativeMethods.cuda_GpuMat_new2(rows, cols, type);
            if (p == IntPtr.Zero)
                throw new OpenCvSharpException();
            InitSafeHandle(p);
        }
        ...
        public GpuMat(int rows, int cols, MatType type, IntPtr data, long step)
        {
            ...
            if (cols <= 0)
                throw new ArgumentOutOfRangeException(nameof(cols));
            var p = NativeMethods.cuda_GpuMat_new3(rows, cols, type, data, (ulong)step);
            if (p == IntPtr.Zero)
                throw new OpenCvSharpException();
            InitSafeHandle(p);
        }
        ...
        {
            ThrowIfNotAvailable();
            var p = NativeMethods.cuda_GpuMat_new6(size, type);
            if (p == IntPtr.Zero)
                throw new OpenCvSharpException();
            InitSafeHandle(p);
        }
        ...
        {
            ThrowIfNotAvailable();
            var p = NativeMethods.cuda_GpuMat_new7(size, type, data, (ulong)step);
            if (p == IntPtr.Zero)
                throw new OpenCvSharpException();
            InitSafeHandle(p);
        }
        ...
        public GpuMat(int rows, int cols, MatType type, Scalar s)
        {
            ...
            if (cols <= 0)
                throw new ArgumentOutOfRangeException(nameof(cols));
            var p = NativeMethods.cuda_GpuMat_new8(rows, cols, type, s);
            if (p == IntPtr.Zero)
                throw new OpenCvSharpException();
            InitSafeHandle(p);
        }
        ...
        {
            ThrowIfNotAvailable();
            var p = NativeMethods.cuda_GpuMat_new11(size, type, s);
            if (p == IntPtr.Zero)
                throw new OpenCvSharpException();
            InitSafeHandle(p);
        }
...
#region Public methods
        ...
        public void ConvertTo(GpuMat dst, MatType rtype, double alpha = 1, double beta = 0)
        {
            ...
            NativeMethods.cuda_GpuMat_convertTo(CvPtr, dst.CvPtr, rtype, alpha, beta);
            GC.KeepAlive(this);
            GC.KeepAlive(dst);
        }
        ...
        public void AssignTo(GpuMat m, MatType type)
        {
            ...
            if (m is null)
                throw new ArgumentNullException(nameof(m));
            NativeMethods.cuda_GpuMat_assignTo(CvPtr, m.CvPtr, type);
            GC.KeepAlive(this);
            GC.KeepAlive(m);
        }
        ...
        {
            ThrowIfDisposed();
            NativeMethods.cuda_GpuMat_create1(CvPtr, rows, cols, type);
            GC.KeepAlive(this);
        }
        ...
        {
            ThrowIfDisposed();
            NativeMethods.cuda_GpuMat_create2(CvPtr, size, type);
            GC.KeepAlive(this);
        }
