...
/// blittable-element VectorOfXxx wrappers (construction, Size, ElemPtr, ToArray).
/// </summary>
public class StdVectorTest : TestBase
{
    [Fact]
    public void VectorOfByte_RoundTrip()
    {
        var data = new byte[] { 1, 2, 3, 255, 0 };
        using var vec = new VectorOfByte(data);
    ...
    [Fact]
    public void VectorOfInt32_RoundTrip()
    {
        var data = new[] { -1, 0, 1, int.MaxValue, int.MinValue };
        using var vec = new VectorOfInt32(data);
        Assert.Equal(data.Length, vec.Size);
        Assert.Equal(data, vec.ToArray());
    }

    [Fact]
    public void VectorOfFloat_RoundTrip()
    {
        var data = new[] { 0f, 1.5f, -2.25f, 100f };
        using var vec = new VectorOfFloat(data);
        Assert.Equal(data, vec.ToArray());
    }

    [Fact]
    public void VectorOfDouble_RoundTrip()
    {
        var data = new[] { 0.0, 1.5, -2.25, 1e10 };
        using var vec = new VectorOfDouble(data);
        Assert.Equal(data, vec.ToArray());
    }
    ...
    {
        using var vec = new VectorOfInt32((nuint)4);
    ...
    [Fact]
    public void VectorOfPoint_RoundTrip()
    {
        var data = new[] { new Point(1, 2), new Point(-3, 4), new Point(5, -6) };
        using var vec = new VectorOfPoint(data);
        var dst = vec.ToArray();
    ...
    [Fact]
    public void VectorOfPoint2f_RoundTrip()
    {
        var data = new[] { new Point2f(1.5f, 2.5f), new Point2f(-3.5f, 4.5f) };
        using var vec = new VectorOfPoint2f(data);
        Assert.Equal(data, vec.ToArray());
    }

    [Fact]
    public void VectorOfPoint3f_RoundTrip()
    {
        var data = new[] { new Point3f(1, 2, 3), new Point3f(-4, -5, -6) };
        using var vec = new VectorOfPoint3f(data);
        Assert.Equal(data, vec.ToArray());
    }

    [Fact]
    public void VectorOfRect_RoundTrip()
    {
        var data = new[] { new Rect(0, 0, 10, 20), new Rect(5, 5, 1, 1) };
        using var vec = new VectorOfRect(data);
        Assert.Equal(data, vec.ToArray());
    }

    [Fact]
    public void VectorOfRect2d_RoundTrip()
    {
        var data = new[] { new Rect2d(0.5, 1.5, 10, 20), new Rect2d(2, 3, 4, 5) };
        using var vec = new VectorOfRect2d(data);
        Assert.Equal(data, vec.ToArray());
    }

    [Fact]
    public void VectorOfRotatedRect_RoundTrip()
    {
        var data = new[]
        {
        ...
        using var vec = new VectorOfRotatedRect(data);
        var dst = vec.ToArray();
        Assert.Equal(data.Length, dst.Length);
    ...
    [Fact]
    public void VectorOfKeyPoint_RoundTrip()
    {
        var data = new[]
        {
        ...
        using var vec = new VectorOfKeyPoint(data);
        Assert.Equal(data, vec.ToArray());
    }

    [Fact]
    public void VectorOfDMatch_RoundTrip()
    {
        var data = new[] { new DMatch(0, 1, 0.5f), new DMatch(2, 3, 1.5f) };
        using var vec = new VectorOfDMatch(data);
        Assert.Equal(data, vec.ToArray());
    }

    [Fact]
    public void VectorOfVec4f_RoundTrip()
    {
        var data = new[] { new Vec4f(1, 2, 3, 4), new Vec4f(-5, -6, -7, -8) };
        using var vec = new VectorOfVec4f(data);
        Assert.Equal(data, vec.ToArray());
    }

    [Fact]
    public void VectorOfVec4i_RoundTrip()
    {
        var data = new[] { new Vec4i(1, 2, 3, 4), new Vec4i(-5, -6, -7, -8) };
        using var vec = new VectorOfVec4i(data);
        Assert.Equal(data, vec.ToArray());
    }
    ...
    {
        using var v2 = new VectorOfVec2f();
        using var v3 = new VectorOfVec3f();
        using var v6f = new VectorOfVec6f();
        using var v6d = new VectorOfVec6d();
        using var p2d = new VectorOfPoint2d();
