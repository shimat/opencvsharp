...
Namespace OpenCvSharpSamplesVB
    ...
    Friend Module DFT
        ...
        Private Sub RunDFT(ByVal srcImg As IplImage)
            ...
            Using realInput As IplImage = Cv.CreateImage(srcImg.Size, BitDepth.F64, 1)
                Using imaginaryInput As IplImage = Cv.CreateImage(srcImg.Size, BitDepth.F64, 1)
                    Using complexInput As IplImage = Cv.CreateImage(srcImg.Size, BitDepth.F64, 2)
                        ...
                        Using dft_A As CvMat = Cv.CreateMat(dftM, dftN, MatrixType.F64C2)
                            Using imageRe As New IplImage(New CvSize(dftN, dftM), BitDepth.F64, 1)
                                Using imageIm As New IplImage(New CvSize(dftN, dftM), BitDepth.F64, 1)
                                    ...
                                    Using TempCvWindow As CvWindow = New CvWindow("Image", WindowMode.AutoSize, srcImg)
                                        Using TempCvWindowMagnitude As CvWindow = New CvWindow("Magnitude", WindowMode.AutoSize, imageRe)
                                            Cv.WaitKey(0)
                                        End Using
                                    End Using
                                End Using
