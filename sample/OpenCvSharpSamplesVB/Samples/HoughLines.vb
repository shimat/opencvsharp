...
Namespace OpenCvSharpSamplesVB
    ...
    Friend Module HoughLines
        ...
        ''' </summary>
        Private Sub SampleCpp()
            ' (1)画像の読み込み 
            Using imgGray As New Mat([Const].ImageGoryokaku, LoadMode.GrayScale)
                Using imgStd As New Mat([Const].ImageGoryokaku, LoadMode.Color)
                    Using imgProb As Mat = imgStd.Clone()
                        ' ハフ変換のための前処理 
                        CvCpp.Canny(imgGray, imgGray, 50, 200, ApertureSize.Size3, False)

                        ' (3)標準的ハフ変換による線の検出と検出した線の描画
                        Dim segStd() As CvLineSegmentPolar = CvCpp.HoughLines(imgGray, 1, Math.PI / 180, 50, 0, 0)
                        ...
                        Dim segProb() As CvLineSegmentPoint = CvCpp.HoughLinesP(imgGray, 1, Math.PI / 180, 50, 50, 10)
                        ...
                        Using TempCvWindow As CvWindow = New CvWindow("Hough_line_standard", WindowMode.AutoSize, imgStd.ToIplImage())
                            Using TempCvWindowProb As CvWindow = New CvWindow("Hough_line_probabilistic", WindowMode.AutoSize, imgProb.ToIplImage())
                                CvWindow.WaitKey(0)
                            End Using
                        End Using
                    End Using
        ...
        Private Sub SampleC()
            ...
            ' (1)画像の読み込み 
            Using srcImgGray As New IplImage([Const].ImageGoryokaku, LoadMode.GrayScale)
                Using srcImgStd As New IplImage([Const].ImageGoryokaku, LoadMode.Color)
                    Using srcImgProb As IplImage = srcImgStd.Clone()
                        ' (2)ハフ変換のための前処理 
                        ...
                        ' (5)検出結果表示用のウィンドウを確保し表示する
                        Using TempCvWindow As CvWindow = New CvWindow("Hough_line_standard", WindowMode.AutoSize, srcImgStd)
                            Using TempCvWindowProb As CvWindow = New CvWindow("Hough_line_probabilistic", WindowMode.AutoSize, srcImgProb)
                                CvWindow.WaitKey(0)
                            End Using
                        End Using
                    End Using
