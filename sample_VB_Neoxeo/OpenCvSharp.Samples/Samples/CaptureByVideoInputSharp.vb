    ...
    Friend Module CaptureByVideoInputSharp
        Public Sub Start()
            ...
            Using vi As New VideoInput()
                ...
                Using img As New IplImage(width, height, BitDepth.U8, 3)
                    Using bitmap As New Bitmap(width, height, PixelFormat.Format24bppRgb)
                        Using form As New Form() With {.Text = "VideoInputSharp sample", .ClientSize = New Size(width, height)}
                            Using pb As New PictureBox() With {.Dock = DockStyle.Fill, .Image = bitmap}
